...
void hmac_sha256_init(hmac_ctx *h, const uint8_t *key, size_t key_len) {
  ...
  sha256_init(&h->ctx);
  sha256_update(&h->ctx, ipad, 64);
}

void hmac_sha256_update(hmac_ctx *h, const void *data, size_t len) {
  sha256_update(&h->ctx, data, len);
}

void hmac_sha256_final(hmac_ctx *h, uint8_t out[32]) {
  uint8_t inner[32];
  sha256_final(&h->ctx, inner);
  sha256_init(&h->ctx);
  sha256_update(&h->ctx, h->opad, 64);
  sha256_update(&h->ctx, inner, 32);
  sha256_final(&h->ctx, out);
}

/* 一次性 HMAC：薄封装流式 API（DRY）。 */
void hmac_sha256(const uint8_t *key, size_t key_len,
                 const uint8_t *msg, size_t msg_len, uint8_t out[32]) {
  hmac_ctx h;
  hmac_sha256_init(&h, key, key_len);
  hmac_sha256_update(&h, msg, msg_len);
