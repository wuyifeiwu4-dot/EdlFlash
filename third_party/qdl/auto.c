...
static void auto_bind_inner(struct qdl_device_auto *wrap, struct qdl_device *inner)
{
	...
	/* Forward the bound transport's serial so the outer device the rest of
	 * qdl talks to also carries it (the OnePlus token handshake reads it). */
	snprintf(wrap->base.serial, sizeof(wrap->base.serial), "%s", inner->serial);
