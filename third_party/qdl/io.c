...
struct qdl_device *qdl_init(enum QDL_DEVICE_TYPE type)
{
	...
	if (type == QDL_DEVICE_AUTO)
		return auto_init();
#else
	if (type == QDL_DEVICE_QUD || type == QDL_DEVICE_AUTO)
		return usb_init();
#endif
...
/**
 * qdl_clear_pending() - Discard any stashed pushback bytes
 * @qdl: device handle
 *
 * Used when the transport session is torn down (e.g. a USB port reset that
 * re-enumerates the device) so leftover bytes from the dead session can't
 * surface on the fresh session's first qdl_read().
 */
...
/**
 * qdl_reset() - Issue a USB port reset (software replug) on the transport
 * @qdl: device handle
 *
 * Used to recover a device whose bulk endpoints are wedged after a previous
 * session (claim/clear_halt succeed but transfers never reach the firmware).
 * The reset restarts the on-device PBL so it re-sends its Sahara HELLO.
 *
 * Returns: 0 on success (handle still valid, re-read the fresh HELLO)
 *	    -ENODEV if the device re-enumerated (handle dead, re-open needed)
 *	    -ENOSYS if the transport can't reset
